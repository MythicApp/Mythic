...
public final class SparkleUpdateControllerModel: NSObject, SPUUserDriver, ObservableObject {
    ...
    @MainActor public static let shared = SparkleUpdateControllerModel()
    ...
    @MainActor public override init() {
        ...
        self.manageBackgroundTask(AppSettingsModel.shared.settings.sparkleUpdateAction != .off)
        AppSettingsModel.shared.$settings
            ...
            backgroundTask = AnyCancellable(BackgroundEventServiceModel.shared.queue
                .schedule(after: .init(.now()), interval: .seconds(60 * 60 * 6), {
                    DispatchQueue.main.async(qos: .background) {
                        SparkleUpdateControllerModel.shared.checkForUpdates(userInitiated: false)
                    }
                }))
        } else {
    ...
    private func getUpdaterAction() -> AppSettingsModel.Settings.AutoUpdateAction {
        if Thread.isMainThread {
            var action: AppSettingsModel.Settings.AutoUpdateAction = .off
            MainActor.assumeIsolated {
                action = AppSettingsModel.shared.settings.sparkleUpdateAction
            }
            return action
        }
        var action: AppSettingsModel.Settings.AutoUpdateAction = .off
    ...
    private func preferSilent() -> Bool {
        if Thread.isMainThread {
            var action = false
            MainActor.assumeIsolated {
                action = AppSettingsModel.shared.settings.inOnboarding
            }
            return action
        }
